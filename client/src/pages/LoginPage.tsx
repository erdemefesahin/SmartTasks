...
function LoginPage({ dark, toggle }: { dark: boolean; toggle: () => void }) {
  ...
  return (
    <div className="flex min-h-screen items-center justify-center px-4 py-8">
      ...
      <div className="w-full max-w-md rounded-3xl bg-white p-8 shadow-lg dark:bg-slate-900 dark:shadow-slate-900/50">
        <h1 className="text-3xl font-semibold text-slate-900 dark:text-slate-100">SmartTasks Login</h1>
        <p className="mt-2 text-sm text-slate-600 dark:text-slate-400">Access your AI-powered task dashboard.</p>
        ...
        <p className="mt-6 text-center text-sm text-slate-600 dark:text-slate-400">
          New to SmartTasks?{' '}
          <Link to="/register" className="font-medium text-slate-900 hover:underline dark:text-slate-200">
            Create an account
          </Link>
        </p>
      </div>
