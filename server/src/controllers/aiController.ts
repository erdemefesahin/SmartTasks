...
export async function enhanceTaskController(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    const outputAny = response as any;
    const rawText =
    ...
    const subtasks = Array.isArray(parsed.subtasks) ? parsed.subtasks.map((item: any) => ({ title: String(item).trim() })) : [];
